...
* 🔢 **Quantidade de Item:** Itens mantêm um saldo numérico real em vez de uma linha por unidade coletada. A **Quantidade por coleta** de um drop controla quantas unidades ele concede a cada coleta (independente do limite de coleta), quests podem premiar qualquer quantidade do item, e trocas podem exigir/entregar qualquer quantidade por item. Saldos altos são exibidos de forma compacta (ex.: `1.5k`) na aba Coleção, nos detalhes do item e no histórico de auditoria.
