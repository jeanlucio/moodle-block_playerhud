...
| `manage/tab_reports_test.php` | 11 | Aba Relatórios: uma instância sem jogadores/itens/quests ainda exporta um resumo bem formado com o drill-down de auditoria inativo; `display()` renderiza HTML real de ponta a ponta pelo `$OUTPUT` global; mais de 30 logs de IA só exportam a primeira página (30) por padrão, mais recentes primeiro, e `ai_showall=1` retorna todos; o fallback de `inventory.source` sem lang string `report_src_*` correspondente é repassado cru ao template como `detail_text` no drill-down de auditoria, e o `tab_reports.mustache` é comprovado que escapa via duplo-mustache na renderização; o seletor de estudante da exportação omite o email de um estudante quando oculto pela política de identidade do site; o KPI "item mais coletado" e o total de itens de cada estudante somam as duas gerações de armazenamento |
