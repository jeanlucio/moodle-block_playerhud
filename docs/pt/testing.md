...
|-----------------|------:|----------------|
| `ai/generator_test.php` | 5 | `save_item()` (via reflection, sem rede): limita um nome gerado por IA acima do tamanho; converte campos não-string antes de persistir; sanitiza uma descrição maliciosa; `is_safe_url()` rejeita um host que não resolve e um literal de IP não-canônico (guarda de SSRF) |
| `ai/hub_usage_reporting_test.php` | 2 | Reporta toda tentativa de IA que falhou quando uma chave do hub é usada, não só a última; nunca reporta uso de uma chave própria do plugin (fora do hub) |
| `backup_restore_test.php` | 3 | Definições de backup/restore cobrem todas as tabelas RPG; round-trip completo de curso (incl. duas atividades reais) preserva dados de classe/capítulo/história RPG, poderes de item (`action_type`/`action_value`), tiers de emoji da classe, e o requisito de uma quest `TYPE_SPECIFIC_TRADE` remapeado contra a troca restaurada em vez do mapeamento de item; um cmid fixado no `deadline_extension` e o requisito de uma quest `TYPE_ACTIVITY` são ambos remapeados para a atividade do curso restaurado; um segundo item `deadline_extension` com seu próprio cmid confirma que o lookup em lote de `after_restore()` não mistura o cmid de um item com o de outro |
| `collection_tab_test.php` | 17 | Aba Coleção: mapeamento de `filter_type` (avatar/prazo/nenhum), `power_hint_avatar` exibido para item não-secreto não possuído e oculto para secreto, flag `is_equipped`; classificação de origem para a fonte de uma linha de inventário (map é reconhecida como própria do PlayerHUD; qualquer coisa fora das 4 fontes conhecidas cai para uma origem genérica "game"); `get_lp_activities()` é memoizado entre múltiplos itens na mesma renderização (N+1 corrigido); `has_power()` é falso pra `action_type` não reconhecido e pra `deadline_extension` sem o plugin companheiro instalado; uma contagem de origem grande renderiza compacta; um item possuído só pelo motor novo de quantidade (sem linhas legadas de inventário) ainda conta e ganha o badge "novo"; um saldo dividido entre as duas gerações de armazenamento soma corretamente num único card; uma quantidade alta renderiza pelo `utils::format_compact_number()`; `get_lp_activities()` exclui uma atividade oculta mesmo com regra de penalidade por atraso habilitada, enquanto uma visível com a mesma regra continua aparecendo; um item de prazo que fixa um único cmid oculto resolve pra uma lista de atividades vazia em vez de vazar o nome dela |
...
| `db_access_test.php` | 1 | O `riskbitmask` declarado de `block/playerhud:manage` inclui `RISK_PERSONAL`, já que o painel de gerenciamento expõe XP, inventário e atribuição de classe de outros usuários |
| `db_upgrade_test.php` | 9 | Passos de upgrade com lógica real de migração de dados (diferente de DDL de schema puro): marca um item pré-existente literalmente chamado 'PlayerCoin' com `action_type=playercoin`, deixa intocados itens não relacionados/já marcados; retroalimenta `xpawarded` numa linha de inventário pré-existente de uma origem que paga XP de verdade (map/teacher/revoked) usando o XP atual do item, mas não para uma linha de origem `drop` nem uma linha coletada de um drop infinito (`maxusage=0`); retroalimenta o `xpawarded` de uma reivindicação de quest pré-existente a partir do `reward_xp` atual da quest; retroalimenta um código de drop faltante com um código único que não colide com um já existente na mesma instância; o passo de upgrade adiciona as tabelas `block_playerhud_stack`/`block_playerhud_stack_log` e as colunas de quantidade de drop/quest sem tocar em nenhuma linha existente; adiciona a coluna `timerevoked` em `block_playerhud_stack_log` sem tocar em nenhuma linha existente |
| `drop_guard_test.php` | 11 | Limites de coleta, itens consumidos por troca, aplicação de cooldown; um limite de coleta é aplicado a partir de um saldo mantido só no log do motor novo de quantidade; um limite combinando linhas legadas de inventário e linhas do log novo bloqueia corretamente; um saldo combinado das duas fontes abaixo do limite ainda é permitido; um cooldown é aplicado a partir da coleta mais recente do motor novo, não só de uma linha legada de inventário |
| `form/edit_item_form_test.php` | 4 | Validação server-side do campo de imagem do item: valor emoji passa; URL HTTPS válida passa; payload de quebra de atributo (`http` seguido de aspas/tags) é rejeitado; valor `http`-like malformado que não é uma URL de verdade é rejeitado |
...
| `privacy_provider_test.php` | 24 | LGPD com cobertura completa: descoberta de contexto/usuário (`get_contexts_for_userid`, `get_users_in_context`); `export_user_data` nas seis subárvores (perfil, RPG, inventário, missões, trocas, logs de IA), incl. `xp_gained` de missão e `created`/`modified` do progresso de RPG, que antes ficavam de fora; exclusão por usuário, multiusuário e de contexto inteiro com garantia de isolamento, incl. a limpeza de `wizard_objects`/`wizard_shortcodes` antes de `wizard_runs` (evitando linhas órfãs com `runid` inexistente); exportação/exclusão de toda chave de API e preferência de avatar; declaração de metadados completa por coluna real da tabela (não por chave individual) para `playerhud_user`, `inventory`, `ai_logs`, `wizard_runs`, `rpg_progress`, `quest_log` e `trade_log`; guardas de contexto não-bloco como no-ops; a declaração de metadados também é checada contra toda coluna real das novas tabelas `block_playerhud_stack` e `block_playerhud_stack_log`; `get_contexts_for_userid()` ainda encontra um usuário cujo único dado mora numa única tabela, e sua contagem de queries de descoberta se mantém estável não importa quantas tabelas do plugin realmente têm dado desse usuário (guarda N+1); uma requisição de contexto alcança a checagem da tabela de concessão dentro de `add_contextlist_for_user()` mesmo pra um usuário sem nenhuma linha em `playerhud_user` |
| `quest_test.php` | 45 | Verificações de conclusão (nível, XP, itens, trocas, conclusão de atividade); reivindicar recompensas; quest desabilitada; idempotência; flags de comemoração de level-up e vitória no jogo ao reivindicar recompensa; `has_claimable_quests` em todos os tipos de requisito incl. conclusão de atividade, com curto-circuito de reivindicadas/não reivindicadas; `preload_totals()` bate com os resultados não-agrupados do próprio `check_status()` por quest em todo tipo apoiado em query, incl. duas quests `TYPE_SPECIFIC_ITEM`/`TYPE_SPECIFIC_TRADE` com ids diferentes, e sua contagem de leituras se mantém estável conforme a lista de quests cresce em vez de escalar; `has_claimable_quests()` mantém suas próprias contagens por tipo isoladas entre itens/trocas e nunca paga por um agregado de um tipo que uma quest anterior na lista já curto-circuitou antes de alcançar; mapeamento de `build_record_from_suggestion`, transporte de item-ids e piso do override de XP; `get_heuristic_suggestions` milestones de nível/coleção/economia/atividade com pulo de duplicatas; uma atividade com acompanhamento de conclusão oferecida como missão heurística é detectada como cumprida assim que a atividade é realmente concluída; `TYPE_SPECIFIC_ITEM`/`TYPE_UNIQUE_ITEMS`/`TYPE_TOTAL_ITEMS` são satisfeitos por um saldo mantido só no motor novo de quantidade, não só em linhas legadas de inventário; reivindicar uma recompensa concede o `reward_itemqty` configurado da quest, não sempre exatamente uma unidade; a mensagem de recompensa mantém o espaçamento do conector intacto |
| `rpg_classes_test.php` | 3 | Consulta de classe (`get_player_class`) e limites de tier de retrato |
| `story_manager_test.php` | 31 | Carregamento de cena, persistência de progresso, navegação de escolhas, delta de karma, conclusão de capítulo, casos de erro; a consequência `set_class_id` de uma escolha atribui a classe RPG do jogador — o único caminho de atribuição — e dispara `character_selected`; `make_choice()` adquire e libera o lock por usuário sem travar numa chamada normal; inventário `consumed`/`revoked` é rejeitado como pagamento de uma escolha com custo, uma cópia genuína é aceita e consumida (e soft-consumida — vira `source=consumed`, não é excluída, então a linha sobrevive); `load_scene()`, `make_choice()` e `load_recap()` revalidam `unlock_date`/`required_level` do capítulo no servidor (antes só a UI bloqueava o clique), com `load_recap()` isento dessa checagem para um capítulo já concluído — reler algo legitimamente terminado nunca re-trava, mesmo que o professor aperte as regras depois; gastar um item numa escolha de história nunca devolve uma coleta do drop finito/com cooldown de onde o item veio, já que o `drop_guard` continua contando a linha (agora consumida, não excluída) contra o `maxusage` do drop; um custo de escolha é aceito quando o pagamento vem só do saldo do motor novo de quantidade; um custo de escolha é aceito quando o pagamento é dividido entre as duas gerações de armazenamento |
...
| `trade_test.php` | 11 | Montagem de trocas, fundos insuficientes, sucesso atômico, limite único, restrição por grupo; uma troca que referencia um item de recompensa desabilitado é rejeitada de imediato mesmo com saldo suficiente; uma troca de quantidade alta concede/consome pelo motor de quantidade sem inserir uma linha de inventário por unidade; o custo de uma troca é pago corretamente quando dividido entre as duas gerações de armazenamento |
...
| Arquivo de teste | Casos | O que é coberto |
|-----------------|------:|----------------|
| `chat_message_test.php` | 4 | Sem chave de API → `moodle_exception`, assegurando o errorcode específico `accessdenied` em vez de qualquer `moodle_exception` onde relevante (uma chave de API não configurada também lançaria uma e mascararia uma checagem de vínculo ausente); campo `message` com HTML sujo é rejeitado; guarda de capability (`manage`); um `courseid` incompatível é rejeitado antes de qualquer chamada de IA ser tentada |
| `collect_item_test.php` | 5 | Item coletado + registro de inventário criado; drop inválido → `success=false`; limite atingido → `success=false`; guarda de capability (`view`); um `courseid` incompatível é rejeitado |
| `create_avatar_pack_test.php` | 7 | 17 itens criados; ids e nomes retornados em lockstep; todos com `action_type=avatar`; deduplicação por emoji; segunda chamada cria 0 (idempotência); guarda de capability; um `courseid` incompatível é rejeitado |
| `create_class_pack_test.php` | 8 | Cria 3 classes; tiers de HP base conforme esperado; pula nome já existente; segunda chamada cria 0 (idempotência); tons diferentes geram nomes diferentes; tom desconhecido cai no fallback fantasia; guarda de capability; um `courseid` incompatível é rejeitado |
| `create_playercoin_test.php` | 4 | Novo item criado; segunda chamada retorna existente (idempotência); guarda de capability; um `courseid` incompatível é rejeitado |
| `execute_chat_action_test.php` | 5 | `action_open_tab` retorna URL de redirect (determinístico, sem IA); tipo de ação desconhecido → `success=false`; parâmetros inválidos → `success=false`; guarda de capability; um `courseid` incompatível é rejeitado |
| `generate_ai_content_test.php` | 4 | Sem chave de API → `success=false`; campo `message` com HTML sujo é rejeitado; guarda de capability (`manage`); um `courseid` incompatível é rejeitado |
| `generate_class_oracle_test.php` | 4 | Sem chave de API → `success=false`; campo `message` com HTML sujo é rejeitado; guarda de capability (`manage`); um `courseid` incompatível é rejeitado |
| `generate_story_test.php` | 4 | Sem chave de API → `success=false`; campo `message` com HTML sujo é rejeitado; guarda de capability (`manage`); um `courseid` incompatível é rejeitado |
| `insert_drop_shortcode_test.php` | 11 | Shortcode inserido no campo de conteúdo do módulo; inserção duplicada rejeitada; drop de outra instância rejeitado; drop renomeado pra atividade em que caiu; `mode=text` com rótulo customizado; modo desconhecido cai pra card; guarda de capability; `execute_batch()` insere shortcodes para vários drops numa só chamada (usada pela distribuição do assistente), e uma falha isolada num drop não bloqueia o processamento dos demais; dois drops caindo na mesma atividade e campo no mesmo lote sobrevivem ambos (o cache de valores de campo pré-carregado é atualizado in-place a cada escrita, não lido uma vez só pro lote inteiro); a contagem de leituras do lote se mantém estável conforme cresce em vez de escalar com o número de itens |
...
| `remove_drop_shortcode_test.php` | 7 | Shortcode existente removido; shortcode separado por `<br>` removido; shortcode com atributos `mode=`/`text=` removido; ausência de shortcode é noop sem erro; guarda de capability; `execute_batch()` remove o shortcode de vários drops numa só chamada reconstruindo o cache do curso uma única vez; a variante em lote tem sua própria guarda de capability |
...
| `use_item_test.php` | 13 | Guarda de capability (`view`, assegurada via uma capability dedicada `interact`); item não possuído → exceção; poder de prazo: sem atividade, sem regra, cria override e consome item, atualiza override existente, rejeita um `targetcmid` de outro curso mesmo quando já existe um override pra esse cmid+userid, rejeita um cmid estrangeiro antes mesmo de consultar a tabela de regra do plugin companheiro, e rejeita um `targetcmid` que o usuário atual não pode ver (atividade oculta) com a mesma mensagem exata de "sem regra configurada", rodado como estudante em vez de admin pra realmente exercitar a checagem de visibilidade; duas concessões sequenciais legítimas (dois itens em posse) consomem cada uma sua própria unidade e estendem exatamente um bloco de dias por chamada, sem dobrar a extensão de nenhuma das duas; poder de avatar: equipar e desequipar funcionam mesmo com `$OUTPUT` resetado pro placeholder `bootstrap_renderer` ainda não resolvido, reproduzindo a mesma condição de uma requisição AJAX real, e equipar continua funcionando pra um avatar concedido só pelo motor novo de quantidade |
...
| `wizard_generate_helpers_test.php` | 13 | `build_step_types()` bate com os módulos selecionados na ordem, pula `auto_distribute` quando o distribuir de Itens está desligado, vazio quando nada selecionado; `compute_shared_xp_shares()` vazio sem Itens/Missões, Pill/Extensão de Prazo usam seus próprios padrões sozinhos, dividem o orçamento com Itens quando combinados; `resolve_or_create_progress_item()` idempotente e cria um item completo quando falta; `resolve_previous_chapter_context()` lê o capítulo mais recente; `distribute_drops()` limita cada atividade à sua cota calculada em vez de deixar só a correspondência de nome empilhar todo drop numa única atividade, espalha entre todas as atividades elegíveis pulando labels; um id de drop de outra instância é silenciosamente excluído em vez de distribuído, sem vazar seu nome nem inflar a contagem de cota; uma lista de drop ids vazia retorna cedo em vez de chegar no `get_in_or_equal()` sem nada pra casar |
...
| `wizard_run_step_test.php` | 57 | Um passo de progresso ao vivo por vez, por mecânica (PlayerCoin, Avatares, Missões, Comércio, Colecionável de Conhecimento, Item Secreto, Ranking, Extensão de Prazo, RPG, Item RPG, auto-distribuir): criação de item/quest/troca com registro no manifesto, retentativas idempotentes, desfazer por mecânica, controle pela flag de distribuir, tom/tamanho de jornada influenciando o conteúdo, e a inserção exclusiva no fórum de avisos pra PlayerCoin e Item Secreto (incl. no-op sem fórum de avisos); tipo de passo desconhecido, guarda de capability, rejeição de `runid` de outra instância, passo com falha não finaliza a rodada, passo final reporta a economia só quando solicitado; um `courseid` que não é o dono de fato da instância do bloco é rejeitado |
| `wizard_start_test.php` | 9 | Um passo de plano por módulo selecionado; a flag de "passo lento" reflete se Próximo Capítulo foi selecionado; a divisão de cotas de XP bate com os módulos selecionados; o XP bônus da Pill presente quando selecionada sozinha; o módulo de arco da história se expande num outline + um passo por capítulo, a quantidade de passos cresce com o tamanho da jornada, o manifesto mantém o nome lógico do módulo; guarda de capability; um `courseid` que não é o dono de fato da instância do bloco é rejeitado |
| **Subtotal** | **180** | |
...
| `chapters_test.php` | 17 | Persistência e ordenação de capítulos: salvar (inserir, atualizar, padrões, isolamento), excluir em cascata cenas/escolhas, mover/reordenar com renumeração da lista completa, no-op na borda; `bulk_delete_chapters()` remove vários capítulos junto com suas cenas e escolhas numa só passada, filtra um capítulo de outra instância em vez de excluí-lo ou lançar exceção, e é no-op para lista de ids vazia; mover um capítulo não escala o número de escritas com o total de capítulos |
...
| `drops_test.php` | 19 | Persistência de drop: salvar (inserir + código, ilimitado, atualizar preserva propriedade, isolamento, item estrangeiro); excluir único e no-op estrangeiro; exclusão em massa só dos próprios com contagem, entrada vazia; `get_owned_item` retorna para a instância dona e rejeita instância estrangeira; `get_sort_data` alterna ícone/direção da coluna de ordenação ativa; `view_manage_page`/`handle_edit_form` renderizam de ponta a ponta pelo `$OUTPUT`/`$PAGE` globais reais para um professor com a capability de gerenciamento; o template da tabela de gerenciamento escapa o conteúdo de mídia do drop; `view_manage_page` ordena corretamente por nome e cai numa coluna segura diante de um parâmetro de ordenação desconhecido em vez de interpolá-lo no SQL; a tela do assistente de distribuir drops escapa o atributo de dados dos cmids inseridos, e o template da sidebar escapa a URL de gerenciamento |
| `export_test.php` | 8 | Construtor da exportação de notas: campos da linha e nível derivado, ordenação por XP, teto de nível, exclusão de professores/gerentes, colunas localizadas sem jogadores, exclusão de não matriculados, desempate por última ação; o email de um estudante é omitido da exportação quando oculto pela política de identidade do site |
| `items_test.php` | 26 | Ciclo de vida do item: toggle de ativação e no-op estrangeiro; conceder adiciona inventário + XP, 0 XP, rejeição estrangeira, e rejeita um usuário que não está de fato matriculado no curso da instância; revogar desconta XP, preserva drop infinito, no-op estrangeiro, é idempotente numa segunda revogação (XP descontado só uma vez), e é no-op numa cópia já consumida; revogar desconta o XP realmente registrado no momento da concessão, não o XP atual do item; detecção de trocas sobreviventes (troca aparada, órfã excluída, não relacionada ignorada); `find_xp_impact` soma só as cópias que realmente ganharam XP entre todos os detentores, vazio para item nunca possuído, e no-op para lista de ids vazia; conceder com um `$qty` explícito concede várias unidades pelo motor de quantidade numa só chamada; revogar uma entrada de `block_playerhud_stack_log` desconta o saldo e o XP registrado, limita a remoção ao saldo atual, é no-op numa linha de `consume`, é idempotente numa segunda revogação da mesma entrada (mantendo intactas as unidades de outras concessões do mesmo usuário), deixa a concessão de origem sem sinalizar quando seu saldo já foi todo gasto, e é no-op numa entrada de instância estrangeira |
| `manage_entry_points_test.php` | 34 | A metade HTTP dos controladores, exercitada pelo ciclo real de requisição (superglobais preenchidas como um navegador faria, `redirect()` capturado como `redirecterrordetected` sob CLI): as ações de excluir e excluir em lote de drops realmente removem as linhas, um id de drop de outra instância nunca é excluído, uma sesskey errada não exclui nada, a listagem renderiza só os drops da própria instância e cai num campo de ordenação seguro diante de um parâmetro `sort` forjado; excluir uma cena cascateia para suas escolhas, um nó de outro capítulo fica intacto, um capítulo de outra instância é rejeitado; coletar concede o item e seu XP, não paga XP em drop infinito, rejeita drop de outra instância, item desabilitado e sesskey inválida sem gravar nada; os editores de classe e troca rejeitam um registro de outra instância, e todas essas telas ficam fechadas para quem não tem `block/playerhud:manage` (ou `:view` no coletar); uma requisição genuinamente submetida (com o marcador `_qf__` do moodleform, como um POST real carrega) para uma troca de outra instância ainda é rejeitada antes de seus requisitos/recompensas serem lidos, fechando um oráculo que vazaria as contagens de linha de uma troca alheia no dimensionamento do formulário re-renderizado; um GET simples com `repeats_req`/`repeats_give` acima do normal é limitado em vez de alocar um número ilimitado de elementos de formulário, e `scenes::clamp_repeats()` limita um valor de `repeats` fornecido pelo cliente ao mesmo teto fixo; a página de drops, o coletar, e os editores de capítulos/classe/troca/cenas rejeitam cada um um `courseid` que não é o dono de fato da instância do bloco; os editores de troca e drop escapam um nome de item malicioso embutido no select de itens e no cabeçalho da página |
| `quests_test.php` | 13 | Ciclo de vida da missão: toggle e no-op estrangeiro; excluir reverte XP por conclusão, sem recompensa, no-op estrangeiro; excluir e excluir em lote revertem o XP realmente registrado por conclusão, não a recompensa atual da quest; massa só dos próprios com reversão agregada de XP e contagem, entrada vazia; `find_xp_impact` soma só as conclusões que realmente ganharam XP entre todos os reivindicantes, vazio para quest nunca reivindicada, e no-op para lista de ids vazia; a prévia de impacto de XP de uma exclusão em lote exclui uma quest de outra instância assim que a lista de ids é escopada à instância |
...
| `trades_test.php` | 10 | Persistência de troca: salvar (inserir com reqs + recompensas, atualizar substitui, isolamento, item estrangeiro filtrado); excluir em cascata reqs/recompensas/log, isolamento, irmãos preservados; `bulk_delete_trades()` remove várias trocas junto com seus requisitos, recompensas e log numa só passada, filtra uma troca de outra instância em vez de excluí-la ou lançar exceção, e é no-op para lista de ids vazia |
| **Subtotal** | **148** | |
...
- `lib.php` (`tests/lib_test.php`): as quatro funções são testadas diretamente —
  ...
  seção de perfil, capability `block/playerhud:view`/`moodle/block:view` negada, opt-out
  `ranking_visibility`), `block_playerhud_get_drop_details_by_code()` (correspondência, código
  ...
  abaixo.
- `db/upgrade.php` (`db_upgrade_test.php`): os 9 testes que carregam lógica real de migração de
