...
| `db_upgrade_test.php` | 9 | Upgrade steps that carry real data-migration logic (as opposed to plain schema DDL): tags a pre-existing item literally named 'PlayerCoin' with `action_type=playercoin`, leaves unrelated/already-tagged items untouched; backfills `xpawarded` on a pre-existing inventory row from a paid source (map/teacher/revoked) using the item's current XP, but not for a `drop`-sourced row nor a row collected from an infinite drop (`maxusage=0`); backfills a pre-existing quest-log claim's `xpawarded` from the quest's current `reward_xp`; backfills a missing drop code with a unique code that does not collide with one already present in the same instance; the upgrade step adds the `block_playerhud_stack`/`block_playerhud_stack_log` tables and the drop/quest quantity columns without touching any existing row; adds the `timerevoked` column to `block_playerhud_stack_log` without touching any existing row |
...
| `privacy_provider_test.php` | 24 | GDPR full coverage: context/user discovery (`get_contexts_for_userid`, `get_users_in_context`); `export_user_data` across all six subtrees (profile, RPG, inventory, quests, trades, AI logs), incl. a quest's `xp_gained` and RPG progress's `created`/`modified`, which were previously left out; per-user, multi-user and whole-context deletion with isolation guarantees, incl. cleaning up `wizard_objects`/`wizard_shortcodes` before `wizard_runs` (no rows left dangling with a nonexistent `runid`); export/delete of every API-key and avatar preference; metadata declaration checked against every real column of `playerhud_user`, `inventory`, `ai_logs`, `wizard_runs`, `rpg_progress`, `quest_log` and `trade_log` (not just a handful of hand-picked keys); non-block context guards are no-ops; metadata declaration is checked against every real column of the new `block_playerhud_stack` and `block_playerhud_stack_log` tables too; `get_contexts_for_userid()` still finds a user whose only data lives in a single table, and its discovery query count stays flat no matter how many of the plugin's tables actually hold data for that user (N+1 guard); a context request reaches the grant-table check inside `add_contextlist_for_user()` even for a user with no `playerhud_user` row at all |
...
| `collect_item_test.php` | 5 | Item collected + inventory record created; invalid drop → `success=false`; limit reached → `success=false`; capability guard (`view`); a mismatched `courseid` is rejected |
| `create_avatar_pack_test.php` | 7 | 17 items created; ids and names returned in lockstep; all have `action_type=avatar`; emoji deduplication; second call creates 0 (idempotency); capability guard; a mismatched `courseid` is rejected |
| `create_class_pack_test.php` | 8 | Creates 3 classes; base-HP tiers match expectations; skips an already-existing name; second call creates 0 (idempotency); different tones produce different names; unknown tone falls back to fantasy; capability guard; a mismatched `courseid` is rejected |
| `create_playercoin_test.php` | 4 | New item created; second call returns existing item (idempotency); capability guard; a mismatched `courseid` is rejected |
| `execute_chat_action_test.php` | 5 | `action_open_tab` returns redirect URL (deterministic, no AI); unknown action type → `success=false`; invalid params → `success=false`; capability guard; a mismatched `courseid` is rejected |
| `generate_ai_content_test.php` | 4 | No API key → `success=false`; an unclean-HTML `message` field is rejected; capability guard (`manage`); a mismatched `courseid` is rejected |
| `generate_class_oracle_test.php` | 4 | No API key → `success=false`; an unclean-HTML `message` field is rejected; capability guard (`manage`); a mismatched `courseid` is rejected |
| `generate_story_test.php` | 4 | No API key → `success=false`; an unclean-HTML `message` field is rejected; capability guard (`manage`); a mismatched `courseid` is rejected |
...
| `remove_drop_shortcode_test.php` | 7 | Existing shortcode stripped; `<br>`-separated shortcode stripped; shortcode carrying `mode=`/`text=` attributes stripped; absent shortcode is a no-op success; capability guard; `execute_batch()` strips the shortcode from several drops in one call while rebuilding the course cache only once; the batch variant has its own capability guard |
...
| `use_item_test.php` | 13 | Capability guard (`view`, asserted via a dedicated `interact` capability requirement); not-owned item → exception; deadline power: no activity selected, no rule found, creates override and consumes item, updates existing override, rejects a `targetcmid` from another course even when an override already exists for that cmid+userid pair, rejects a foreign cmid before ever probing the companion plugin's own rule table, and rejects a `targetcmid` the current user cannot see (hidden activity) with the exact same message as "no rule configured", run as a student rather than admin so the visibility check is actually exercised; two legitimate sequential grants (two items held) each consume their own unit and extend by exactly one day-block per call, without either one doubling the extension; avatar power: equip and unequip both succeed with `$OUTPUT` reset to the still-unresolved `bootstrap_renderer` placeholder, reproducing the exact precondition of a real AJAX dispatch, and equipping still works for an avatar granted only through the new quantity engine |
...
| `drops_test.php` | 19 | Drop persistence: save (insert + code, unlimited, update preserves ownership, isolation, foreign item); delete single and foreign no-op; bulk deletes only owned with count, empty input; `get_owned_item` returns for the owning instance and rejects a foreign one; `get_sort_data` toggles the icon/direction for the active sort column; `view_manage_page`/`handle_edit_form` render end to end through the real global `$OUTPUT`/`$PAGE` for a teacher with the manage capability; the management table template escapes drop media content; `view_manage_page` sorts correctly by name and falls back to a safe column for an unknown sort parameter instead of interpolating it into SQL; the distribute-drops wizard screen escapes the inserted-cmids data attribute, and the sidebar view template escapes the manage URL |
...
| `items_test.php` | 26 | Item lifecycle: enable toggle and foreign no-op; grant adds inventory + XP, zero-XP, foreign rejection, and rejects a user not actually enrolled in the instance's course; revoke deducts XP, infinite-drop preservation, foreign no-op, is idempotent on a second revoke (XP only deducted once), and is a no-op on an already-consumed copy; revoke deducts the XP actually recorded at grant time, not the item's current XP; surviving-trade detection (trimmed trade, orphaned excluded, unrelated ignored); `find_xp_impact` aggregates only copies that actually earned XP across all holders, empty for an unheld item, and a no-op for an empty id list; granting with an explicit `$qty` grants multiple units through the quantity engine in one call; revoking a single `block_playerhud_stack_log` entry decrements the balance and deducts its recorded XP, caps the removal at the current balance, is a no-op for a `consume` row, is idempotent on a second revoke of the same entry (leaving units from the same user's other grants untouched), leaves the source grant unflagged once its balance is already fully spent, and is a no-op for a foreign-instance entry |
...
- `external\use_item` (82%) has an atomic race-guard branch (consuming the item before writing
...
- `lib.php` (`tests/lib_test.php`): all four functions are tested directly —
  ...
  below.
- `db/upgrade.php` (`db_upgrade_test.php`): the 9 tests carrying real data-migration logic call
