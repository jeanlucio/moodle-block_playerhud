...
/**
 * Tests for the generate_story web service (validation and error paths).
 *
...
/**
 * Tests for the generate_story web service.
 *
 * @package    block_playerhud
 * @copyright  2026 Jean Lúcio
 * @license    https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @covers     \block_playerhud\external\generate_story
 */
final class generate_story_test extends external_base_testcase {
    /**
     * With no API key the call returns success=false instead of throwing.
     */
    public function test_generate_story_without_key_returns_failure(): void {
        set_config('apikey_gemini', '', 'block_playerhud');
        set_config('apikey_groq', '', 'block_playerhud');
        set_config('apikey_openai', '', 'block_playerhud');

        $result = generate_story::execute($this->instanceid, $this->course->id, 'A haunted forest');

        $this->assertFalse($result['success']);
        $cleaned = external_api::clean_returnvalue(generate_story::execute_returns(), $result);
