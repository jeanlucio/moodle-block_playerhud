...
final class wizard_run_step_test extends external_base_testcase {
    ...
    /**
     * The "pill" step creates the tone-specific Pill and Book items and spreads Pill drops
     * across every eligible activity per drop_distribution::compute_activity_quotas() — records
     * everything into the run's manifest. Deliberately creates no "collect them all" quest (see
     ...
     */
    public function test_pill_step_creates_items_and_distributes_with_manifest(): void {
        global $DB;
        ...
        $this->assertTrue($result['success']);
        $this->assertSame(2, $result['counts']['items']);
        $this->assertSame(1, $result['counts']['trades']);
        $this->assertSame(1, $result['counts']['quests']);
    ...
    /**
     * Unchecking Pill's own "distribute into activities" flag still creates both items, but
     * skips the quota-scatter across course activities entirely — proving distribute actually
     * gates generate_pill()'s own placement, independent of every other module.
     */
    public function test_pill_step_distribute_false_skips_activity_scatter(): void {
        global $DB;
     ...
     */
    public function test_pill_step_run_can_be_rolled_back(): void {
        global $DB;
    ...
    /**
     * Running the "pill" step twice, in two different runs, must not duplicate the item pair or
     * its drops.
     */
    public function test_pill_step_is_idempotent_across_runs(): void {
        $firstrunid = $this->start_empty_run();
        ...
        $this->assertSame(2, $first['counts']['items']);
        ...
        $this->assertSame(0, $second['counts']['items']);
        $this->assertSame(0, $second['counts']['quests']);
        $this->assertSame(0, $second['counts']['trades']);
    }

    /**
     * The "pill" step wires the intrinsic Pill<->Book trade and its completion quest —
     * independent of the separate "comercio" step being run at all.
     */
    public function test_pill_step_wires_book_trade_and_quest(): void {
        global $DB;
     ...
     * non-tradable, and plants a single one-time drop for it in the course's news forum, in
     * mode=text with a generic emoji label so it blends into the forum's own intro instead of
     * showing up as an obvious card.
     */
    public function test_secret_drops_step_creates_item_and_plants_drop_in_news_forum(): void {
        global $DB;
        ...
        $forumintro = $DB->get_field('forum', 'intro', ['id' => $forum->id]);
        $this->assertStringContainsString('[PLAYERHUD_DROP code=' . $drop->code . ' mode=text text="💻"]', $forumintro);
        $this->assertStringContainsString('Welcome to the course!', $forumintro);
        ...
        $this->assertSame('intro', $shortcoderow->field);
    }

    /**
     * When the course has no news forum, the item is still created but no drop is planted —
     * same tolerance as generate_playercoin()'s own "no forum" case.
     */
    public function test_secret_drops_step_creates_item_but_no_drop_without_news_forum(): void {
        global $DB;
     ...
     * skips planting its drop in the news forum entirely.
     */
    public function test_secret_drops_step_distribute_false_skips_forum_drop(): void {
        global $DB;
    ...
    /**
     * Rolling back a "secret_drops" run removes the item and drop, and strips the shortcode back
     * out of the news forum's intro.
     */
    public function test_secret_drops_step_run_can_be_rolled_back(): void {
        ...
        $this->assertEquals(0, $DB->count_records('block_playerhud_drops', ['blockinstanceid' => $this->instanceid]));
        $introafter = $DB->get_field('forum', 'intro', ['id' => $forum->id]);
        $this->assertStringNotContainsString('PLAYERHUD_DROP', (string) $introafter);
    }
    ...
    /**
     * The "latepenalty" step creates the Deadline Extension item and an early "reach level 2"
     * quest that rewards it, recording both into the run's manifest. Requires local_latepenalty.
     */
    public function test_latepenalty_step_creates_item_and_quest_with_manifest(): void {
        global $DB;
    ...
    /**
     * Running the "latepenalty" step twice, in two different runs, must not duplicate the item
     * or quest.
     */
    public function test_latepenalty_step_is_idempotent_across_runs(): void {
        if (!class_exists('\local_latepenalty\recalculator')) {
            $this->markTestSkipped('Requires local_latepenalty.');
        }
     ...
     */
    public function test_latepenalty_step_run_can_be_rolled_back(): void {
        global $DB;
        if (!class_exists('\local_latepenalty\recalculator')) {
            $this->markTestSkipped('Requires local_latepenalty.');
        }
    ...
    /**
     * Without local_latepenalty installed, the step is a clean no-op — no item, no quest. Only
     * meaningful (and only runs) in an environment where the plugin is absent, the inverse of
     * every other Latepenalty test in this file.
     */
    public function test_latepenalty_step_noop_when_not_installed(): void {
        if (class_exists('\local_latepenalty\recalculator')) {
            $this->markTestSkipped('Only relevant when local_latepenalty is absent.');
        }

        $runid = $this->start_empty_run();
        $result = wizard_run_step::execute($this->instanceid, $this->course->id, $runid, 'latepenalty', '');

        $this->assertTrue($result['success']);
        $this->assertSame(0, $result['counts']['items']);
        $this->assertSame(0, $result['counts']['quests']);
    }

    /**
     * The "latepenalty" step wires the PlayerCoin<->Deadline Extension trade as part of
     * generating the item, when PlayerCoin already exists — no separate "comercio" step
     * required. Requires local_latepenalty (for the item).
     */
    public function test_latepenalty_step_wires_playercoin_trade(): void {
        global $DB;
        if (!class_exists('\local_latepenalty\recalculator')) {
            $this->markTestSkipped('Requires local_latepenalty.');
        }
